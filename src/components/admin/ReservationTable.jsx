    ...
    <div className="overflow-x-auto">
      <table className="w-full table-auto border-separate border-spacing-y-2">
        <thead>
          ...
          <th className="w-[12%] px-3 py-3">Reference</th>
          <th className="w-[18%] px-3 py-3">Room</th>
          <th className="w-[8%] px-3 py-3">Guest</th>
          <th className="w-[16%] px-3 py-3">Stay</th>
          <th className="w-[8%] px-3 py-3">Status</th>
          <th className="w-[7%] px-3 py-3 text-right">Total</th>
          <th className="w-[6%] px-3 py-3">Eco</th>
          ...
          {bookings.map((booking) => {
            ...
            return (
              <tr key={booking.id} className="rounded-2xl bg-slate-50/80 align-top shadow-[0_1px_0_rgba(15,23,42,0.04)]">
                <td className="px-3 py-4 text-[12px] max-w-[140px] overflow-hidden">
                  <span className="inline-flex rounded-full bg-white px-2.5 py-1 font-mono font-semibold text-slate-700 ring-1 ring-slate-200 truncate">
                    {booking.reference || `BK-${booking.id}`}
                  </span>
                </td>
                <td className="px-3 py-4">
                  <p className="text-sm font-medium text-[#1A1A2E] truncate max-w-[240px]">{room?.roomTypeName || `Room #${booking.roomId}`}</p>
                  <p className="mt-1 text-[11px] text-[#6B7280] truncate max-w-[240px]">{room?.branchName || '—'}</p>
                </td>
                <td className="px-3 py-4 text-[12px] text-[#6B7280] truncate">#{booking.userId}</td>
                ...
                <td className="px-3 py-4">
                  <span className={`inline-flex rounded-full px-2 py-0.5 text-[11px] font-semibold ${statusClasses[booking.status] || 'bg-slate-100 text-slate-600'}`}>
                    {booking.status}
                  </span>
                </td>
                <td className="px-3 py-4 text-[12px] font-semibold text-[#1A1A2E] text-right truncate max-w-[120px]">
                ...
                <td className="px-3 py-4 text-[12px] text-[#6B7280] truncate max-w-[140px]">
                  {booking.createdAt ? format(parseISO(booking.createdAt), 'd MMM yyyy, HH:mm') : '—'}
                </td>
                <td className="px-3 py-4">
                  <div className="flex flex-wrap items-center gap-2 max-w-[140px]">
                    {canConfirm ? (
                      <button
                        type="button"
                        ...
                        className="flex cursor-pointer items-center gap-1 rounded-full border border-[#0A7C6E] px-2 py-1 text-xs font-semibold text-[#0A7C6E] transition hover:bg-[#E6F5F3] disabled:cursor-not-allowed disabled:opacity-50"
                      >
                    ...
                    {canCancel ? (
                      <button
                        ...
                        className="flex cursor-pointer items-center gap-1 rounded-full border border-[#F5C2C7] px-2 py-1 text-xs font-semibold text-[#9B1E1E] transition hover:bg-[#FEF3F3] disabled:cursor-not-allowed disabled:opacity-50"
                      >
